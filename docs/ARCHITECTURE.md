...
Embeddings are generated by Ollama (default: `nomic-embed-text`, 768 dimensions). The query embedding is compared against all node embeddings via approximate nearest neighbors. An Ollama health check (200ms timeout, 30s cache) gates this channel -- if Ollama is down, retrieval falls back to FTS-only.
