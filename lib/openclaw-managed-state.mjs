...
export function captureOpenClawManagedState(configPath, fsImpl = fs) {
  ...
  const slotsMemory = String(parsed?.plugins?.slots?.memory || "").trim();
  if (slotsMemory) {
    snapshot.plugins.slotsMemory = slotsMemory;
  }
  ...
  const hasManagedState = snapshot.requiredAllow.length > 0
    ...
    || Object.keys(snapshot.plugins).length > 0;
...
export function reconcileOpenClawManagedStateObject(currentConfig, snapshot) {
  ...
  if (isRecord(snap.plugins)) {
    const slotsMemory = String(snap.plugins.slotsMemory || "").trim();
    if (slotsMemory) {
      if (!isRecord(parsed.plugins.slots)) parsed.plugins.slots = {};
      if (String(parsed.plugins.slots.memory || "").trim() !== slotsMemory) {
        parsed.plugins.slots.memory = slotsMemory;
  ...
  if (isRecord(snap.agents)) {
    ...
    if (Object.keys(listPrimaries).length > 0) {
      ...
      for (const [agentId, primary] of Object.entries(listPrimaries)) {
        if (!agentId || !primary) continue;
        const existing = parsed.agents.list.find((entry) => isRecord(entry) && String(entry.id || "").trim() === agentId);
        if (!existing) continue;
        if (!isRecord(existing.model)) existing.model = {};
        if (String(existing.model.primary || "").trim() !== primary) {
          existing.model.primary = primary;
...
export function restoreOpenClawManagedState(configPath, snapshot, fsImpl = fs) {
  const cfgPath = String(configPath || "").trim();
  if (!cfgPath) {
    return { changed: false, reason: "missing-config-path", changedBits: [] };
  }
  if (!snapshot || !isRecord(snapshot)) {
    return { changed: false, reason: "missing-snapshot", changedBits: [] };
  }

  const raw = fsImpl.readFileSync(cfgPath, "utf8");
  const parsed = JSON.parse(raw);
  const reconciled = reconcileOpenClawManagedStateObject(parsed, snapshot);
  if (!reconciled.changed) {
    return { changed: false, reason: "already-matches", changedBits: [] };
  }

  writeJsonAtomically(fsImpl, cfgPath, reconciled.config);
  return { changed: true, changedBits: reconciled.changedBits };
}

export function readOpenClawManagedStateSnapshot(snapshotPath, fsImpl = fs) {
  const filePath = String(snapshotPath || "").trim();
