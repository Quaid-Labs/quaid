...
function writeSessionCursorToEnd(sessionId: string, transcriptPath: string, agentLabel: string = "main"): void {
  const sid = String(sessionId || "").trim();
  const resolvedPath = String(transcriptPath || "").trim();
  if (!sid || !resolvedPath) return;
  try {
    const cursorPath = agentLabel && agentLabel !== "main"
      ? sessionCursorPathForAgent(sid, agentLabel)
      : sessionCursorPath(sid);
    ...
    console.warn(
      `[quaid][cursor] writeSessionCursorToEnd failed for session=${sid}: ${String((err as Error)?.message || err)}`
    );
    if (isFailHardEnabled()) throw err;
  }
}

function isInstancePreservedSessionTranscript(transcriptPath: string, instanceRoot: string): boolean {
  const resolvedPath = String(transcriptPath || "").trim();
...
function shouldRepairRollingCursorToLiveTranscript(
  cursorPath: string,
  transcriptPath: string,
  instanceRoot: string,
): boolean {
...
function seedRollingCursorForTranscript(
  sessionId: string,
  transcriptPath: string,
  agentLabel: string = "main",
  ...
  try {
    ...
    const repairingPreservedMirror = cursorExists
      && shouldRepairRollingCursorToLiveTranscript(cursorPath, resolvedPath, instanceRoot);
  ...
  try {
    ...
    for (const name of cursorNames) {
      ...
      try {
        ...
        console.warn(
          `[quaid][cleanup] failed advancing internal cursor ${cursorPath}: ${String((err as Error)?.message || err)}`
        );
      }
    }
  } catch (err: unknown) {
    if (!isMissingFileError(err)) {
      console.warn(
        `[quaid][cleanup] failed scanning internal session cursor dir ${cursorDir}: ${String((err as Error)?.message || err)}`
      );
    }
  }
  ...
  try {
    ...
    for (const name of signalNames) {
      ...
      try {
        ...
        console.warn(
          `[quaid][cleanup] failed pruning internal signal ${signalPath}: ${String((err as Error)?.message || err)}`
        );
      }
    }
  } catch (err: unknown) {
    if (!isMissingFileError(err)) {
      console.warn(
        `[quaid][cleanup] failed scanning internal signal dir ${signalDir}: ${String((err as Error)?.message || err)}`
      );
    }
  }
  ...
  try {
    ...
    for (const name of cursorNames) {
      ...
      try {
        ...
        console.warn(
          `[quaid][cleanup] failed repairing cursor ${cursorPath}: ${String((err as Error)?.message || err)}`
        );
        if (isFailHardEnabled()) throw err;
      }
    }
    ...
    console.warn(
      `[quaid][cleanup] failed scanning cursor repairs in ${cursorDir}: ${String((err as Error)?.message || err)}`
    );
    if (isFailHardEnabled()) throw err;
  }
  if (repaired) {
...
function shouldKeepRicherPreservedTranscript(destPath: string, sourcePath: string, reason: string): boolean {
...
function buildRefreshedIdentityContext(instanceId: string, maxChars: number = REFRESHED_IDENTITY_CONTEXT_MAX_CHARS): string {
  const normalizedInstance = String(instanceId || "").trim();
  ...
  for (const filename of IDENTITY_CONTEXT_FILES) {
    ...
    try {
      ...
      console.warn(`[quaid] Refreshed identity context skipped ${filename}: ${(err as Error)?.message || String(err)}`);
...
// OC enforces the registration timeout outside this handler. Keep it above
// the internal recall deadline plus sync docs/model-context work so prompt
// dispatch waits for the returned injection payload.
const BEFORE_PROMPT_BUILD_HOOK_TIMEOUT_MS = 60_000;
const BEFORE_PROMPT_BUILD_IN_FLIGHT_TIMEOUT_MS = _envTimeoutMs(
  "QUAID_BEFORE_PROMPT_BUILD_IN_FLIGHT_TIMEOUT_MS",
  Math.max(BEFORE_PROMPT_BUILD_HOOK_TIMEOUT_MS, PYTHON_BRIDGE_TIMEOUT_MS) + 5_000,
);
...
function deferredNoticeRelayStableTurnKey(agentLabel: string, event: any, ctx: any, promptSessionId: string): string {
  const sessionScope = firstNonEmptyString(
    event?.sessionKey,
    ctx?.sessionKey,
    event?.targetSessionKey,
...
function consumeDeferredNoticeRelayContextForReply(turnKeys: string[]): string {
  let relay = "";
  const seen = new Set<string>();
...
function extractDeferredNoticeMessagesFromRelayContext(context: string): string[] {
...
function formatImmediateProviderNoticeContext(message: string): string {
  const notice = String(message || "").trim();
  if (!notice) {
    return "";
  }
  return (
    "MANDATORY: Quaid has an active provider/configuration error for the human user. "
    + "Start your next response by relaying this exact Quaid error before any other answer.\n\n"
    + "<quaid_system_message>\n"
    + `• ${notice}\n`
    + "</quaid_system_message>"
  );
...
type AutoInjectTurnOutcome = {
  ...
  skipReason?: "auto_inject_disabled" | "low_quality_query" | "in_flight_timeout";
};
...
function _withBeforePromptBuildInFlightTimeout(
  turnPromise: Promise<AutoInjectTurnOutcome>,
  turnKey: string,
  query: string,
  startedAtMs: number = Date.now(),
): Promise<AutoInjectTurnOutcome> {
  let timeoutTimer: ReturnType<typeof setTimeout> | undefined;
  const timeoutPromise = new Promise<AutoInjectTurnOutcome>((resolve) => {
...
function _trackBeforePromptBuildInFlightTurn(
  turnKey: string,
  query: string,
  turnPromise: Promise<AutoInjectTurnOutcome>,
...
function createAdapterFacade(instanceId: string = _QUAID_INSTANCE): AdapterFacade {
  ...
  return createQuaidFacade({
    ...
    execDocsRegistry: (cmd, args) =>
      _spawnWithTimeout(DOCS_REGISTRY, "registry", [cmd, ...args], "docs_registry", instanceEnv),
...
const quaidPlugin = {
  ...
  register(api: OpenClawPluginApi<PluginConfig>) {
    ...
    const embeddedPromptBuildFallbackSessionScope = (event: any, ctx: any): string => {
    ...
    const embeddedPromptBuildFallbackSessionGuardKey = (agentLabel: string, event: any, ctx: any): string => {
    ...
    const markEmbeddedPromptBuildFallbackTurn = (turnKey: string, sessionGuardKey: string = ""): void => {
      const key = String(turnKey || "").trim();
      if (!key) return;
      const nowMs = Date.now();
    ...
    const consumeEmbeddedPromptBuildFallbackTurn = (turnKey: string, sessionGuardKey: string = ""): boolean => {
      const key = String(turnKey || "").trim();
      const guardKey = String(sessionGuardKey || "").trim();
      const nowMs = Date.now();
      const consumeTurnKey = (candidate: string): boolean => {
...
`);
    ...
    const consumeRefreshedIdentityContext = (refreshKeys: string[], instanceId: string): string => {
    ...
    const drainRefreshedIdentityContext = (refreshKeys: string[], instanceId: string, source: string): void => {
      const keys = Array.from(
        new Set(
          refreshKeys
            .map((key) => String(key || "").trim())
    ...
    const peekRefreshedIdentityContext = (refreshKeys: string[], instanceId: string, targetHook: string): string => {
      const keys = Array.from(
        new Set(
          refreshKeys
            .map((key) => String(key || "").trim())
    ...
    const armLifecycleProjectContextRefresh = (
      ...
      // Refreshed identity files are system context, so /new must re-arm them
      // even when project-doc injection is disabled for the instance.
      const projectsEnabled = isSystemEnabled("projects");
    ...
    beforePromptBuildHandler = async (event: any, ctx: any): Promise<HookContextResult | undefined> => {
      ...
      if (
        !event?.__quaidEmbeddedPromptBuildFallback
        && consumeEmbeddedPromptBuildFallbackTurn(embeddedFallbackTurnKey, embeddedFallbackSessionGuardKey)
      ) {
      ...
      const injectFilePlacementReminder = (sessionKeyDocs: string): void => {
        ...
        if (rawText.length >= 3 && !rawText.startsWith("/")) {
          ...
          if (sessionId) {
            ...
            const resolvedSessionKey = String(
              (transientOrigin ? currentInteractiveSession?.key : "")
              || resolveSessionKeyForSessionId(sessionId)
              || originSessionKey
              || ""
            ).trim();
            ...
            if (
              preservedPath
              && isSystemEnabled("memory")
              && !isInternalSessionContext(sessionContext, sessionContext)
            ) {
              seedRollingCursorForTranscript(
                sessionId,
                preservedPath,
