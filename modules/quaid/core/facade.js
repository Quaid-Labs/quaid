...
function isDocsProjectRecallItem(item) {
...
function selectAutoInjectionMemories(rows, limit) {
  ...
  const candidates = [...projectRows].filter((item) => !seen.has(recallItemIdentity(item))).sort((a, b) => Number(b?.similarity || 0) - Number(a?.similarity || 0));
...
function createQuaidFacade(deps) {
  ...
  const knowledgeEngine = createKnowledgeEngine({
    ...
    getCommandRegistry: () => {
      ...
      const miscPath = instanceName ? path.join(resolveVisibleHome(), "projects", `misc--${instanceName}`) : null;
  ...
  function maybeForceCompactionAfterTimeout(sessionId) {
    ...
    const enabled = Boolean(
      captureCfg.compact_on_timeout ?? captureCfg.autoCompactionOnTimeout ?? true
    );
    if (!enabled) return;
  ...
  async function processLifecycleEvent(signal, context) {
    ...
    if (label === "TimeoutSignal") {
      ...
      const timeoutSource = String(ctx.timeoutSource || ctx.timeout_source || "").trim();
      if (timeoutSource) payload.timeout_source = timeoutSource;
  ...
  function resolveExtractionTrigger(label) {
    ...
    const hasToken = (token) => new RegExp(`(?:^|[^a-z0-9])${token}(?:[^a-z0-9]|$)`, "i").test(normalized);
...
Consider running: docs staleness updater (update-stale --apply)`;
  ...
  function parseMemoryBridgePayload(output, expandGraph) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const via = item.via || (categoryKind === "source_chunk" || categoryKind === "session_chunk" || sourceKind === "source_chunk" || sourceKind === "session_chunk" ? "session_chunks" : void 0) || (expandGraph ? void 0 : "vector");
        results.push({
          text,
          category,
  ...
  function parseProjectBrokerResults(out, project) {
    ...
    const rows = [];
    const fallbackProject = String(docs.project || project || "").trim() || void 0;
    for (const chunk of chunks) {
      if (!chunk || typeof chunk !== "object") continue;
      const content = String(chunk.content || "").trim().replace(/\s+/g, " ");
      if (!content) continue;
      ...
      const chunkProject = String(chunk.project || fallbackProject || "").trim() || void 0;
      rows.push({
...
${header}${journalContent}` : `${header}${journalContent}`;
  ...
  function _pathContains(base, candidate) {
  ...
  function _projectCatalogSection(projectName, projectDir, docFiles, cwd) {
    const lines = [
