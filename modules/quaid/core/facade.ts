...
export function createQuaidFacade(deps: QuaidFacadeDeps): QuaidFacade {
  ...
  function lexicalTokens(value: string, limit = 16): string[] {
    const seen = new Set<string>();
    const tokens: string[] = [];
...
${lines.join("\n")}
  ...
  function prepareAutoInjectionContext(params: {
    ...
    const currentOwner = resolveOwner(undefined, extractChannelFromSessionKey(sessionKey));
  ...
  function _projectCatalogDocFiles(projectDir: string): string[] {
  ...
  async function injectProjectContext(existingContext?: string, options: ProjectContextOptions = {}): Promise<string | undefined> {
    ...
    try {
      ...
      if (!options.identityOnly) {
        // Project docs: compact catalogs for every visible project.
        const projectsDir = path.join(resolveVisibleHome(), "projects");
        let subdirs: string[] = [];
        try {
