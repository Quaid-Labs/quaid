...
def _identity_context_content_from_rule(filename: str, content: str) -> str:
    ...
    for raw_line in text.splitlines():
        line = raw_line.strip()
        if line.startswith("# Quaid ") and line.endswith((" Rules", " Context")):
            continue
        if line == _RULES_FILE_PROVENANCE:
            continue
...
def _format_rule_file_content(filename: str, sections: List[str]) -> str:
    ...
    return (
        f"# Quaid {title} Context\n\n"
        f"{_RULES_FILE_PROVENANCE}\n\n"
        f"{body}\n"
    )
