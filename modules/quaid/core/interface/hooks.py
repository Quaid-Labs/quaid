...
_GENERATED_USER_SNIPPETS_PROJECTION_RE = re.compile(
    r"\n?<!-- generated by quaid user snippets projection start -->[\s\S]*?<!-- generated by quaid user snippets projection end -->\n?",
    flags=re.IGNORECASE,
)
_GENERATED_USER_SNIPPETS_PROJECTION_MARKER_RE = re.compile(
    r"<!-- generated by quaid user snippets projection start -->",
    flags=re.IGNORECASE,
)
_GENERATED_MEMORY_PROJECTION_BLOCK_RE = re.compile(
    r"\n?<!-- generated by quaid(?: live)? memory projection(?: fallback on [^>]+)? -->[\s\S]*?(?=\n# [^\n]+|\Z)",
    flags=re.IGNORECASE,
)
_GENERATED_MEMORY_PROJECTION_MARKER_RE = re.compile(
...
def _strip_generated_projection_blocks(content: str) -> str:
    """Remove janitor-generated projection blocks from startup rules payloads."""
    if not content:
        return ""
    content = re.sub(_GENERATED_USER_SNIPPETS_PROJECTION_RE, "\n\n", content)
    content = re.sub(_GENERATED_MEMORY_PROJECTION_BLOCK_RE, "\n\n", content)
    return content.strip()


def _contains_generated_projection_marker(content: str) -> bool:
    if not content:
        return False
    if _GENERATED_USER_SNIPPETS_PROJECTION_MARKER_RE.search(content):
        return True
    if _GENERATED_MEMORY_PROJECTION_MARKER_RE.search(content):
        return True
    return False
...
def hook_session_init(args):
    ...
    for special_file in ("USER.md", "SOUL.md", "ENVIRONMENT.md"):
        ...
        if fpath.is_file():
            raw_content = fpath.read_text(encoding="utf-8").strip()
            if _contains_generated_projection_marker(raw_content):
                print(
                    f"[quaid][session-init] skipping projection-derived identity file: {special_file}",
                    file=sys.stderr,
                )
                continue
            content = raw_content
            content = _strip_generated_projection_blocks(content)
            if content:
