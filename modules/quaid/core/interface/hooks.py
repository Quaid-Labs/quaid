...
_LEGACY_IDENTITY_GUIDANCE_HEADERS = {
    "USER.md": "# USER.md Guidance (Quaid)",
    "SOUL.md": "# SOUL.md Guidance (Quaid)",
    "ENVIRONMENT.md": "# ENVIRONMENT.md Guidance (Quaid)",
}
...
def _strip_generated_user_projection_block(text: str) -> str:
    if not text:
        return ""
    if _GENERATED_USER_SNIPPETS_START in text and _GENERATED_USER_SNIPPETS_END not in text:
        return text.split(_GENERATED_USER_SNIPPETS_START, 1)[0].rstrip()
...
def _has_meaningful_identity_prefix(text: str) -> bool:
    lines = [line.strip() for line in str(text or "").splitlines() if line.strip()]
    if not lines:
        return False
    if len(lines) == 1 and lines[0].lower() in {"# memory", "# environment"}:
        return False
    return True


def _strip_generated_memory_projection_block(text: str) -> str:
    """Remove old generated ENVIRONMENT projections while preserving authored text.

    The marker remains useful as a cleanup sentinel in code, but anything after it
    was generated queue/projection state and should not be rendered as identity.
    """
    if not text:
        return ""
    marker_match = _LEGACY_GENERATED_MEMORY_PROJECTION_MARKER_RE.search(text)
    marker_pos = text.find(_GENERATED_MEMORY_PROJECTION_MARKER)
    positions = [pos for pos in [marker_pos, marker_match.start() if marker_match else -1] if pos >= 0]
    if not positions:
        return text.strip()
    prefix = text[: min(positions)].strip()
    return prefix if _has_meaningful_identity_prefix(prefix) else ""


def _identity_context_content(filename: str, content: str) -> str:
    """Return agent-visible authored identity content only."""
    text = str(content or "").strip()
    if not text:
        return ""
    text = _strip_generated_user_projection_block(text).strip()
    text = _strip_generated_memory_projection_block(text).strip()
    if not text:
        return ""
    guidance_header = _LEGACY_IDENTITY_GUIDANCE_HEADERS.get(filename)
    if guidance_header and text.startswith(guidance_header):
        return ""
    return text
