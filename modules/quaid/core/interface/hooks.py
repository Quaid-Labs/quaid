...
_GENERATED_USER_SNIPPETS_BLOCK_RE = re.compile(
    r"\n?<!-- generated by quaid user snippets projection start -->[\s\S]*?<!-- generated by quaid user snippets projection end -->\n?",
    flags=re.IGNORECASE,
)
_GENERATED_ENVIRONMENT_PROJECTION_RE = re.compile(
    r"<!-- generated by quaid memory projection -->",
    flags=re.IGNORECASE,
)
_LEGACY_GENERATED_ENVIRONMENT_PROJECTION_RE = re.compile(
...
def _strip_generated_identity_projection(doc_file: str, content: str) -> str:
    """Remove Quaid-generated identity projection blocks from startup rules.

    Generated snippet/memory projections can lag behind direct DB cleanup and
    should not be treated as canonical startup rules content. Session-start
    rules should carry authored identity/project guidance, while live memory
    comes from recall paths.
    """
    if not content:
        return ""
    if doc_file == "USER.md":
        return re.sub(_GENERATED_USER_SNIPPETS_BLOCK_RE, "\n\n", content).strip()
    if doc_file == "ENVIRONMENT.md":
        match = (
            _GENERATED_ENVIRONMENT_PROJECTION_RE.search(content)
            or _LEGACY_GENERATED_ENVIRONMENT_PROJECTION_RE.search(content)
        )
        if match:
...
def hook_session_init(args):
    ...
    for special_file in ("USER.md", "SOUL.md", "ENVIRONMENT.md"):
        ...
        if fpath.is_file():
            ...
            content = _strip_generated_identity_projection(special_file, content)
            if content:
