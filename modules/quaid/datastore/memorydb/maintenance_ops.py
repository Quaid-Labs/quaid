        ...
        for cand in candidates:
            ...
            if in_similarity_band or subset_overlap_below_band:
                dup_candidates.append({
                    ...
                    "occurred_start_a": getattr(new_node, "occurred_start", None),
                    "occurred_end_a": getattr(new_node, "occurred_end", None),
                    "mentioned_at_a": getattr(new_node, "mentioned_at", None),
                    "id_b": cand.id,
                    "text_b": cand.name,
                    "created_b": cand.created_at,
                    "valid_from_b": cand.valid_from,
                    "valid_until_b": cand.valid_until,
                    "occurred_start_b": getattr(cand, "occurred_start", None),
                    "occurred_end_b": getattr(cand, "occurred_end", None),
                    "mentioned_at_b": getattr(cand, "mentioned_at", None),
...
def backfill_edges(
    ...
    relations_list = "\n".join(
        f"- {r}" for r in [
            ...
            "friend_of", "neighbor_of", "colleague_of", "works_at", "trains_at", "lives_in",
