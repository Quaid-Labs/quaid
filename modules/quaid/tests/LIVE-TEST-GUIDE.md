...
```bash
ssh alfie.local 'cd ~/quaid/dev && QUAID_INSTALL_AGENT=1 QUAID_OWNER_NAME="Solomon" QUAID_INSTANCE=openclaw node setup-quaid.mjs --agent --workspace "/Users/clawdbot/quaid" --source local'
```
...
PY'
ssh alfie.local 'cd ~/quaid/dev && QUAID_INSTALL_AGENT=1 QUAID_OWNER_NAME="Solomon" QUAID_INSTANCE=claude-code QUAID_INSTALL_CLAUDE_CODE=1 node setup-quaid.mjs --agent --claude-code --workspace "/Users/clawdbot/quaid" --source local'
```
