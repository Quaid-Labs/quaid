...
```bash
ssh example.local 'ls ~/quaid/openclaw-main/scratch/ 2>/dev/null && echo "PASS: file in instance scratch" || echo "FAIL: instance scratch empty or missing"'
ssh example.local 'ls ~/quaid/scratch/*.py 2>/dev/null && echo "FAIL: leaked to workspace scratch root" || echo "PASS: no leak to workspace root"'
```
