...
| Script | Purpose |
|--------|---------|
| `livetest-presnapshot-preflight.sh` | **Run before overnight loops or when platform drift is suspected.** Clones the current base VM, applies slow platform CLI upgrades, refreshes VM Claude OAuth credentials, bakes required OpenClaw Matrix plugin support (`openclaw >=2026.6.33`, Matrix plugin pinned by default to `@openclaw/matrix@2026.6.1`), runs final harness cleanup such as stale OpenClaw silo pruning, and refreshes the base snapshot only if maintenance changed the clone. |
