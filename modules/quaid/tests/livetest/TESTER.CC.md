...
CC requires a long-lived Anthropic API token at `WORKSPACE/adaptors/claude-code/.auth-token`.
This token is generated by `claude setup-token` (NOT from `~/.claude/.credentials.json`).
...
ssh REMOTE_HOST "mkdir -p ~/.quaid/adaptors/claude-code && cat /tmp/anthtoken.txt | tr -d '[:space:]' > ~/.quaid/adaptors/claude-code/.auth-token && chmod 600 ~/.quaid/adaptors/claude-code/.auth-token"
```
