    ...
    def test_exact_duplicate_caught_by_hash(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            with patch("datastore.memorydb.memory_graph.get_graph") as mock_get:
                ...
                r2 = store("Quaid enjoys espresso coffee", owner_id="quaid", status="approved")
...
class TestFactVersioning:
    ...
    def test_get_fact_history_chain(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            v3 = Node.create(type="Fact", name="Quaid lives in Ubud Bali", owner_id="quaid")
            graph.add_node(v1)
            graph.add_node(v2)
            graph.add_node(v3)
    ...
    def test_double_supersede_noop(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            new2 = Node.create(type="Fact", name="Quaid lives in Japan", owner_id="quaid")
            graph.add_node(old)
            graph.add_node(new1)
            graph.add_node(new2)
...
class TestHealthMetrics:
    ...
    def test_counts_with_data(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            n3 = Node.create(type="Fact", name="Melina is Quaid wife partner", owner_id="quaid", status="pending")
            graph.add_node(n1)
            graph.add_node(n2)
            graph.add_node(n3)
