...
class TestMultiHopTraversal:
    ...
    def test_depth_1_returns_immediate_neighbors(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            c = Node.create(type="Person", name="Levi test person", owner_id="quaid")
            graph.add_node(a)
            graph.add_node(b)
            graph.add_node(c)
    ...
    def test_depth_2_returns_two_hops(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            c = Node.create(type="Person", name="Levi multi hop", owner_id="quaid")
            graph.add_node(a)
            graph.add_node(b)
            graph.add_node(c)
            graph.add_edge(Edge.create(source_id=a.id, target_id=b.id, relation="spouse_of"))
    ...
    def test_early_stopping(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            for i in range(10):
                leaf = Node.create(type="Fact", name=f"Leaf fact number {i} for star graph", owner_id="quaid")
                graph.add_node(leaf)
    ...
    def test_depth_tracking(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            c = Node.create(type="Person", name="Hop two depth test", owner_id="quaid")
            d = Node.create(type="Person", name="Hop three depth test", owner_id="quaid")
            graph.add_node(a)
            graph.add_node(b)
            graph.add_node(c)
            graph.add_node(d)
