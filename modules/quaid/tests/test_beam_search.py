...
def _disable_external_reranker(monkeypatch):
    """Prevent network-backed reranker calls in baseline beam tests.

    Most tests in this module validate traversal/scoring mechanics and should
    never depend on external LLM availability. Adaptive reranker tests patch
...
def _add_node(graph, name, node_type="Fact", confidence=0.8, verified=False,
              storage_strength=5.0, status="active", owner_id="quaid"):
...
class TestBeamScoring:
    ...
    def test_intent_boost_affects_score(self, graph):
        ...
        person_node = _add_node(graph, "Douglas Quaid", node_type="Person", confidence=0.8)
...
class TestBeamConfig:
    ...
    def test_traversal_config_from_json(self):
        ...
        from config import TraversalConfig, reload_config, _config_paths
        ...
        try:
            with patch.object(config_module, "_config_paths", lambda: [Path(config_path)]):
