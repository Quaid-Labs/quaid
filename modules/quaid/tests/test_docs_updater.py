...
class TestDriftDetectionFallback:
    ...
    def test_detect_drift_uses_epoch_until_for_commit_lookup(self, tmp_path, monkeypatch):
        cfg = _make_test_config(
            source_mapping={"src.py": {"docs": ["docs/doc.md"]}},
        )
        calls = []
        ...
        with patch("datastore.docsdb.updater.get_config", return_value=cfg), \
             _adapter_patch(tmp_path) as iroot, \
             patch("datastore.docsdb.updater.subprocess.run", side_effect=_fake_run), \
             patch("datastore.docsdb.updater._compute_staleness_score", return_value=42.0):
            doc = iroot / "docs" / "doc.md"
            src = iroot / "src.py"
            doc.parent.mkdir(parents=True, exist_ok=True)
            doc.write_text("# Doc\n")
        ...
        commit_lookup_calls = [
            cmd
            for cmd in calls
            if cmd[:4] == ["git", "log", "-1", "--format=%H"] and "src.py" in cmd
        ]
        assert commit_lookup_calls
        assert "--until=@100" in commit_lookup_calls[0]
        assert "--until=100" not in commit_lookup_calls[0]
