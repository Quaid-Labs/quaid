...
def _setup_entity_with_facts(graph, entity_name="Hauser", entity_type="Person", owner="quaid"):
...
class TestGetEntitySummary:
    ...
    def test_returns_none_for_nonexistent_node(self, tmp_path):
        from datastore.memorydb.memory_graph import get_entity_summary
    ...
    def test_returns_none_when_no_summary(self, tmp_path):
        from datastore.memorydb.memory_graph import get_entity_summary, Node
        graph, _ = _make_graph(tmp_path)
...
class TestGenerateEntitySummaryNoLLM:
    ...
    def test_returns_none_for_nonexistent_node(self, tmp_path):
        from datastore.memorydb.memory_graph import generate_entity_summary
        graph, _ = _make_graph(tmp_path)
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph):
    ...
    def test_returns_none_for_fact_node(self, tmp_path):
        from datastore.memorydb.memory_graph import generate_entity_summary, Node
        graph, _ = _make_graph(tmp_path)
...
class TestGenerateEntitySummaryWithLLM:
    ...
    def test_uses_llm_when_available(self, tmp_path):
        ...
        graph, _ = _make_graph(tmp_path)
        entity, _ = _setup_entity_with_facts(graph)
        mock_response = "Hauser is a software engineer who loves hiking and has a golden retriever named Max."
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph), \
             patch("datastore.memorydb.memory_graph._HAS_LLM_CLIENTS", True), \
             patch("datastore.memorydb.memory_graph.call_fast_reasoning", return_value=(mock_response, {"usage": {}})):
    ...
    def test_falls_back_on_llm_failure(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph), \
             patch("datastore.memorydb.memory_graph._HAS_LLM_CLIENTS", True), \
             patch("datastore.memorydb.memory_graph.call_fast_reasoning", side_effect=Exception("API error")):
            result = generate_entity_summary(entity.id, use_llm=True)
    ...
    def test_falls_back_when_llm_not_available(self, tmp_path):
        ...
        entity, _ = _setup_entity_with_facts(graph)
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph), \
             patch("datastore.memorydb.memory_graph._HAS_LLM_CLIENTS", False):
            result = generate_entity_summary(entity.id, use_llm=True)
    ...
    def test_llm_summary_stored_in_attributes(self, tmp_path):
        ...
        graph, _ = _make_graph(tmp_path)
        entity, _ = _setup_entity_with_facts(graph)
        mock_response = "Hauser is a software engineer."
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph), \
             patch("datastore.memorydb.memory_graph._HAS_LLM_CLIENTS", True), \
             patch("datastore.memorydb.memory_graph.call_fast_reasoning", return_value=(mock_response, {"usage": {}})):
    ...
    def test_llm_empty_response_falls_back(self, tmp_path):
        ...
        with patch("datastore.memorydb.memory_graph.get_graph", return_value=graph), \
             patch("datastore.memorydb.memory_graph._HAS_LLM_CLIENTS", True), \
             patch("datastore.memorydb.memory_graph.call_fast_reasoning", return_value=("", {"usage": {}})):
            result = generate_entity_summary(entity.id, use_llm=True)
...
class TestSummarizeAllEntities:
    ...
    def test_summarizes_all_entity_types(self, tmp_path):
        from datastore.memorydb.memory_graph import summarize_all_entities, Node
        graph, _ = _make_graph(tmp_path)
        # Create one of each type
        for name, typ in [("Alice", "Person"), ("Portland", "Place"), ("AI", "Concept")]:
            entity = Node.create(type=typ, name=name, owner_id="quaid")
            entity.embedding = _fake_get_embedding(name)
            graph.add_node(entity)
            fact = Node.create(type="Fact", name=f"{name} is interesting", owner_id="quaid")
