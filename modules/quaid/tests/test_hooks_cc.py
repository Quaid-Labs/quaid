...
class TestHookSessionInitRegistryAugmentation:
    ...
    def test_user_generated_snippet_projection_is_stripped(self, tmp_path, monkeypatch):
        """Generated USER snippet projection blocks are removed from startup rules."""
        projects_dir, identity_dir, rules_dir = self._make_init_env(tmp_path, monkeypatch)
        ...
        (identity_dir / "USER.md").write_text(
            "\n".join(
                [
                    ...
                    "",
                    "- Diana started a ceramics studio this spring.",
        ...
        with patch("core.project_registry.list_projects", return_value={}):
            _, _, content = _run_hook_session_init(
                {"session_id": "s-user-strip", "cwd": str(tmp_path)},
                monkeypatch=monkeypatch,
                rules_dir=rules_dir,
            )

        assert content is not None
        assert "Preferred style: concise and direct." in content
    ...
    def test_environment_generated_memory_projection_is_stripped(self, tmp_path, monkeypatch):
        """Generated ENVIRONMENT projection content is not injected into startup rules."""
        projects_dir, identity_dir, rules_dir = self._make_init_env(tmp_path, monkeypatch)
        ...
        (identity_dir / "ENVIRONMENT.md").write_text(
            "\n".join(
                [
                    "# ENVIRONMENT",
                    "",
                    "Host preference: always include absolute paths.",
                    "",
        ...
        with patch("core.project_registry.list_projects", return_value={}):
            _, _, content = _run_hook_session_init(
                {"session_id": "s-env-strip", "cwd": str(tmp_path)},
                monkeypatch=monkeypatch,
                rules_dir=rules_dir,
            )

        assert content is not None
        assert "Host preference: always include absolute paths." in content
