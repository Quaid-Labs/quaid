...
def test_auto_provision_writes_lean_instance_config(tmp_path, monkeypatch):
    ...
    assert set(config.keys()) == {"instance"}
...
def test_get_adapter_infers_type_from_instance_prefix_with_lean_config(tmp_path, monkeypatch):
    ...
    config_path.write_text(json.dumps({"instance": {"id": instance_id}}), encoding="utf-8")
