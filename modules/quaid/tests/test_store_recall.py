...
class TestRecallBasic:
    ...
    def test_fast_recall_default_store_plan_timeout_matches_injection_budget(self):
        ...
        # The store layer honors the configured injection timeout. Caller layers
        # that need a larger floor must pass timeout_ms explicitly.
...
class TestSourceChunkStorage:
    ...
    def test_source_chunk_owner_and_domain_filters_preserve_isolation(self, tmp_path):
        ...
        assert graph.get_source_chunk(private["chunk_id"], owner_id="ada", fail_hard=False) is None
        with pytest.raises(RuntimeError, match="owner mismatch"):
            graph.get_source_chunk(private["chunk_id"], owner_id="ada", fail_hard=True)
        with pytest.raises(RuntimeError, match="owner mismatch"):
...
class TestRecallTelemetry:
    ...
    def test_low_signal_query_expansion_is_language_neutral_noop(self):
        ...
        assert _expand_low_signal_query("Maya app", "PROJECT") == "Maya app"
    ...
    def test_deliberate_recall_routes_relation_chain_planner_graph_through_store_plan(self):
        ...
        def _fake_run(query, *, stores, limit, owner_id, min_similarity, planner_profile, planned_queries, planner_meta, fast_mode, graph_depth, common_kwargs):
            ...
            captured["stores"] = list(stores)
            captured["planned_queries"] = list(planned_queries or [])
            captured["planner_meta"] = dict(planner_meta or {})
            ...
            return (
                [
                    {
                        "id": "terminal-repair-studio",
                        "text": "Owner --spouse_of--> Partner --sibling_of--> Brother --has_fact--> Brother runs a repair studio",
                        "category": "fact",
                        "similarity": 1.0,
                        "graph_path": "Owner --spouse_of--> Partner --sibling_of--> Brother --has_fact--> Brother runs a repair studio",
                    }
                ],
                {"planned_stores": ["vector", "graph"], "store_runs": [{"store": "graph"}]},
                None,
            )
        ...
        with patch.object(mg, "get_graph", return_value=_Graph()), \
             ...
             patch.object(mg, "_recall_once", side_effect=AssertionError("relation-chain graph plan must not fall through to vector-only recall")):
            rows, meta = mg.recall(
                "what does my partners brother do",
                owner_id="solomon",
                return_meta=True,
        ...
        assert rows and "repair studio" in rows[0]["text"]
        assert captured["stores"] == ["vector", "graph"]
        assert captured["planned_queries"] == ["what does my partners brother do"]
        assert captured["fast_mode"] is False
        assert captured["graph_depth"] == 2
        assert meta["planned_stores"] == ["vector", "graph"]

    def test_deliberate_recall_falls_back_when_relation_chain_store_plan_times_out(self):
        import datastore.memorydb.memory_graph as mg
        ...
        def _fake_once(**kwargs):
            ...
            captured["include_graph_traversal"] = kwargs["include_graph_traversal"]
            return (
                [
                    {
                        "id": "fallback-repair-studio",
                        "text": "Yuni's brother runs a repair studio",
                        "category": "fact",
                        "similarity": 0.92,
                    }
                ],
                {"counts": {"final_results": 1}, "phases_ms": {"total_ms": 1}},
            )
        ...
        with patch.object(mg, "get_graph", return_value=_Graph()), \
             ...
             patch.object(mg, "_recall_once", side_effect=_fake_once):
            rows, meta = mg.recall(
                "what does my partners brother do",
                owner_id="solomon",
                return_meta=True,
        ...
        assert rows and rows[0]["id"] == "fallback-repair-studio"
        assert captured["query"] == "what does my partners brother do"
        assert captured["include_graph_traversal"] is True
        fallback = meta["turn_details"][0]["planner"]["store_plan_fallback"]
    ...
    def test_deliberate_recall_keeps_single_relation_graph_plan_on_branch_path(self):
        ...
        def _fake_once(**kwargs):
            ...
            return (
                [
                    {
                        "id": "sibling-fact",
                        "text": "Sibling runs a repair studio",
                        "category": "fact",
                        "similarity": 0.9,
                    }
                ],
        ...
        with patch.object(mg, "get_graph", return_value=_Graph()), \
             ...
             patch.object(
                 ...
                 "_plan_fanout_queries",
                 return_value=(
                     ["what does my sibling do"],
                     {
                         "query": "what does my sibling do",
                         "used_llm": False,
                         "queries_count": 1,
                         "elapsed_ms": 0,
                         "planner_profile": "full",
             ...
             ), patch.object(mg, "_run_recall_store_plan", side_effect=AssertionError("single-relation graph plan must stay on branch recall path")), \
             patch.object(mg, "_recall_once", side_effect=_fake_once):
            rows, meta = mg.recall(
                "what does my sibling do",
                owner_id="solomon",
        ...
        assert rows and "repair studio" in rows[0]["text"]
        assert captured["query"] == "what does my sibling do"
        assert captured["include_graph_traversal"] is True
...
class TestRecallFastHookInjectContract:
    ...
    def test_recall_fast_cli_default_timeout_uses_preinject_floor(self):
        import datastore.memorydb.memory_graph as mg

        assert mg._resolve_cli_recall_fast_timeout_ms(None) == 30_000

    def test_recall_fast_cli_explicit_timeout_stays_authoritative(self):
        import datastore.memorydb.memory_graph as mg

        assert mg._resolve_cli_recall_fast_timeout_ms(3000) == 3000
