...
class TestRecallFastHookInjectContract:
    ...
    def test_vector_store_recall_fast_mode_skips_planner_without_preplanned_queries(self):
        import datastore.memorydb.memory_graph as mg
        ...
        with patch.object(mg, "recall", side_effect=_fake_recall):
            mg._vector_store_recall(
                ...
                planner_meta=None,
                fast_mode=True,
                common_kwargs={"project": "quaid"},
            )

        assert captured["query"] == "What do I know about Baxter?"
        assert captured["kwargs"]["use_routing"] is False
        assert captured["kwargs"]["use_reranker"] is False
        assert captured["kwargs"]["use_multi_pass"] is False
