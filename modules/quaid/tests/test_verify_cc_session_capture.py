...
def _write_binding(
    ...
    *,
    ...
    if project_dir_value is not None:
        payload["project_dir"] = project_dir_value
...
def test_verify_cc_session_capture_rejects_binding_without_project_dir(tmp_path):
    project_dir = tmp_path / "cc-livetest"
    project_dir.mkdir()
    instance_id = "claude-code-private-tmp-cc-livetest"
    home = _prepare_capture_home(tmp_path, project_dir, instance_id)
    _write_binding(home, project_dir, instance_id)

    result = _run_verify(home, project_dir, instance_id)

    assert result.returncode == 1
    assert "skipped missing project_dir" in result.stdout
    assert "binding_instance=(none)" in result.stdout


def test_verify_cc_session_capture_rejects_binding_for_wrong_project_dir(tmp_path):
    project_dir = tmp_path / "cc-livetest"
    project_dir.mkdir()
    wrong_project_dir = tmp_path / "other-project"
    wrong_project_dir.mkdir()
    instance_id = "claude-code-private-tmp-cc-livetest"
    home = _prepare_capture_home(tmp_path, project_dir, instance_id)
    _write_binding(
        home,
        project_dir,
        instance_id,
        project_dir_value=str(wrong_project_dir.resolve()),
    )

    result = _run_verify(home, project_dir, instance_id)

    assert result.returncode == 1
    assert "skipped project mismatch" in result.stdout
    assert "binding_instance=(none)" in result.stdout


def test_verify_cc_session_capture_rejects_binding_without_instance_config(tmp_path):
    project_dir = tmp_path / "cc-livetest"
    project_dir.mkdir()
    instance_id = "claude-code-private-tmp-cc-livetest"
    home = _prepare_capture_home(tmp_path, project_dir, instance_id)
    (home / ".quaid" / "instances" / instance_id / "config.json").unlink()
    _write_binding(
        home,
        project_dir,
        instance_id,
    ...
    assert result.returncode == 1
    assert f"skipped missing config for {instance_id}" in result.stdout
    assert "binding_instance=(none)" in result.stdout
