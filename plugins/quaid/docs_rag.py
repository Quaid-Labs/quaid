...
class DocsRAG:
    ...
    def index_document(self, file_path: str) -> int:
        ...
        if chunks_created > 0:
            try:
                ...
                try:
                    rel = str(Path(file_path).relative_to(WORKSPACE))
    ...
    def search_docs(self, query: str, limit: int = 5, min_similarity: float = 0.3,
        ...
        if project:
            ...
            try:
                ...
                registry_paths = [str(WORKSPACE / d["file_path"]) for d in reg_docs]
            except Exception:
                pass
    ...
    def _get_project_paths(self, project: str) -> dict:
        ...
        try:
            ...
            if defn:
                return {
                    "home_dir": str(WORKSPACE / defn.home_dir),
                    "source_roots": [str(WORKSPACE / r) for r in defn.source_roots],
                }
        except Exception:
            pass
...
def main():
    ...
    reindex_parser.add_argument('--dir', default=str(WORKSPACE), help='Base directory to scan')
    ...
    if args.command == 'reindex':
        ...
        for i, result in enumerate(results, 1):
            source_short = result["source"].replace(str(WORKSPACE) + "/", "~/")
