...
class DocsRegistry:
    ...
    def create_project(
        ...
        rel_path = str(project_md_path.relative_to(WORKSPACE))
        self.register(
            file_path=rel_path,
            project=name,
            asset_type="doc",
    ...
    def rename_project(self, old_name: str, new_name: str) -> Dict[str, Any]:
        ...
        if defn:
            ...
            new_dir = WORKSPACE / "projects" / new_name
    ...
    def archive_project(self, project_name: str) -> Dict[str, Any]:
        ...
        if defn:
            ...
            archive_dir = WORKSPACE / "projects" / "archive"
            if src_dir.exists():
...
def sync_existing_docs(registry: DocsRegistry) -> Dict[str, int]:
    ...
    project_home = WORKSPACE / cfg.projects.definitions.get(
    ...
    for doc_path in sorted(all_doc_paths):
        ...
        title = registry._extract_title_from_path(str(WORKSPACE / doc_path))
...
def _generate_project_md(registry: DocsRegistry, project_name: str, cfg) -> None:
    ...
    project_home = WORKSPACE / defn.home_dir
    ...
    for doc in docs:
        abs_path = str((WORKSPACE / doc["file_path"]).resolve())
