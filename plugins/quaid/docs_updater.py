...
def _load_cleanup_state() -> Dict[str, dict]:
    ...
    if not CLEANUP_STATE_PATH.exists():
        return {}
    try:
        return json.loads(CLEANUP_STATE_PATH.read_text())
    except (json.JSONDecodeError, IOError):
        return {}
...
def _save_cleanup_state(state: Dict[str, dict]) -> None:
    ...
    CLEANUP_STATE_PATH.write_text(json.dumps(state, indent=2))
...
def cleanup_doc(doc_path: str, purpose: str, dry_run: bool = True) -> bool:
    ...
    response, duration = call_high_reasoning(
        prompt=user_message,
...
def update_doc_from_diffs(
    ...
    if classification["confidence"] < 0.6:
        try:
            ...
            gate_response, _ = call_low_reasoning(gate_prompt, max_tokens=50, timeout=10)
...
def detect_changed_sources_from_transcript(transcript: str) -> List[str]:
    ...
    response, duration = call_low_reasoning(prompt, max_tokens=300, timeout=30.0)
    if not response:
