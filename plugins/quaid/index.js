...
const WORKSPACE = process.env.CLAWDBOT_WORKSPACE || "~/<username>/clawd";
...
function getProjectNames() {
    try {
        const output = (0, node_child_process_1.execSync)(`python3 "${DOCS_REGISTRY}" list-projects --names-only`, { encoding: "utf-8", env: { ...process.env, MEMORY_DB_PATH: DB_PATH, CLAWDBOT_WORKSPACE: WORKSPACE }, timeout: 10000 }).trim();
        return output.split("\n").filter(Boolean);
    }
    catch {
...
// ============================================================================
// Status messages to inject on the next turn (for user-facing feedback)
// ============================================================================
...
async function recall(query, limit = 5, currentSessionId, compactionTime, expandGraph = true, graphDepth = 1) {
    try {
...
async function store(text, category = "fact", sessionId, extractionConfidence = 0.5, owner = resolveOwner(), source, speaker, status, privacy, keywords, knowledgeType, sourceType) {
    try {
        ...
        const beforeAgentStartHandler = async (event, ctx) => {
            ...
            try {
                ...
                const visibleMemories = allMemories.filter(m => {
                    if (!m.privacy || m.privacy === "public" || m.privacy === "shared")
                        return true;
                    if (m.privacy === "private")
                        return !m.ownerId || m.ownerId === currentOwner;
                    return true;
                });
                ...
                if (apiKey && candidates.length > 3) {
                    try {
                        const numbered = candidates.map((m, i) => `${i + 1}. [${m.category}] ${m.text}`).join("\n");
...
CRITICAL RULE: When the user mentions someone by name (e.g., "shannon", "Yuni", "Troy"), you MUST include EVERY fact that mentions that person — their job, family, relationships, preferences, everything about them. The assistant needs the full picture of anyone being discussed.
...
Example:
Message: "I'm having dinner with Mom tonight"
Facts about Mom (her name, birthday, preferences, health) → ALL INCLUDED
...
Respond with ONLY valid JSON, no other text.`,
                        ...
                        console.log(`[quaid] Haiku reranker error: ${err.message}`);
                        newMemories = candidates.slice(0, 5);
                    }
                }
...
`);
        ...
        const agentEndHandler = async (_event, _ctx) => {
...
QUERY TIPS: Use names ("Shannon" not "my sister"), be specific ("Wendy birthday" not "mom birthday"). Keep limit low (5) for quality.
...
graphDepth: Set to 2 when you need to infer relationships (e.g., nephew = sibling's child). Default 1 is usually sufficient.`,
            parameters: typebox_1.Type.Object({
                ...
                limit: typebox_1.Type.Optional(typebox_1.Type.Number({ description: "Max results (default: 5, max: 8). Lower is better for quality." })),
                expandGraph: typebox_1.Type.Optional(typebox_1.Type.Boolean({ description: "Traverse relationship graph - use for people/family queries (default: true)" })),
                graphDepth: typebox_1.Type.Optional(typebox_1.Type.Number({ description: "Graph traversal depth (default: 1). Use 2 to infer extended relationships like nephew/cousin." })),
            }),
                        ...
                        try {
                            await Promise.race([
                                ...
                                new Promise((_, reject) => {
                                    timer = setTimeout(() => reject(new Error("extraction timeout")), 60000);
                                }),
...
`);
            ...
            for (const fact of facts) {
                ...
                const knowledgeType = category === "preference" ? "preference" : "fact";
...
`);
            ...
            try {
                ...
                const doExtraction = async () => {
                    ...
                    if (isSystemEnabled("memory") && uniqueSessionId) {
                        ...
                        let logData = {};
                        try {
                            logData = JSON.parse(require('fs').readFileSync(logPath, 'utf8'));
                        }
                        catch { }
        ...
        (0, plugin_sdk_1.registerPluginHttpRoute)({
