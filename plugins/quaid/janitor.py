...
from llm_clients import (call_low_reasoning, call_high_reasoning, call_anthropic,
                         parse_json_response, reset_token_usage, get_token_usage,
                         estimate_cost, ANTHROPIC_TIMEOUT, LOW_REASONING_TIMEOUT)
from notify import notify_janitor_summary, notify_daily_memories

# Configuration — resolved from config system
DB_PATH = get_db_path()
WORKSPACE = Path(os.environ.get("CLAWDBOT_WORKSPACE", "~/<username>/clawd"))
...
HAIKU_BATCH_SIZE = 100  # Legacy safety cap (pairs per low-reasoning call)
...
class TokenBatchBuilder:
    ...
    def __init__(self, model_tier: str = 'high',
        ...
        """
        Args:
            model_tier: 'high' or 'low' — selects context window from config
...
def batch_duplicate_check(pairs: List[Dict[str, Any]], metrics: JanitorMetrics) -> List[Optional[Dict[str, Any]]]:
    """Check a batch of duplicate pairs in a single Haiku call.
...
For each pair, decide: merge (same fact, different wording) or keep_both (actually different).
...
JSON array only:"""
    ...
    response, duration = call_low_reasoning(prompt, max_tokens=200 * len(pairs), timeout=LR_BATCH_TIMEOUT)
...
def batch_contradiction_check(pairs: List[Dict[str, Any]], metrics: JanitorMetrics) -> List[Optional[str]]:
    """Check a batch of contradiction pairs in a single Haiku call.
...
JSON array only:"""
    ...
    response, duration = call_low_reasoning(prompt, max_tokens=150 * len(pairs), timeout=LR_BATCH_TIMEOUT)
    metrics.add_llm_call(duration)
...
def resolve_contradictions_with_opus(graph: MemoryGraph, metrics: JanitorMetrics,
                                     dry_run: bool = True) -> Dict[str, int]:
    """Resolve pending contradictions using Opus for high-reasoning decisions."""
...
JSON array only:"""
        ...
        response, duration = call_high_reasoning(prompt, max_tokens=300 * len(batch))
        metrics.add_llm_call(duration)
...
JSON array only:"""
    ...
    response, duration = call_high_reasoning(prompt, max_tokens=200 * len(facts),
...
def review_dedup_rejections(graph: MemoryGraph, metrics: JanitorMetrics,
    ...
    builder = TokenBatchBuilder(
        ...
        tokens_per_item_fn=lambda e: (
            ...
            estimate_tokens(e.get("haiku_reasoning", "")) + 40
        ),
        max_items=100
    )
    batches = builder.build_batches(pending)
    ...
    for batch_num, batch in enumerate(batches, 1):
        ...
        for i, entry in enumerate(batch):
            numbered.append(
                ...
                f'   Haiku reasoning: {entry.get("haiku_reasoning", "N/A")}'
            )

        prompt = f"""You are reviewing {len(batch)} dedup rejections in a personal knowledge base.

Each entry was rejected as a duplicate during memory storage. Review whether the rejection
was correct (CONFIRM) or a false positive where different information was wrongly blocked (REVERSE).

CONFIRM: The two texts really are the same fact, just reworded. Rejection was correct.
REVERSE: The two texts are genuinely different information. The new text should be stored.
...
[
  ...
  {{"item": 2, "action": "REVERSE", "reason": "different aspects of X"}}
]

JSON array only:"""

        dedup_prompt_tag = f"[prompt:{_prompt_hash(prompt)}] "
        response, duration = call_high_reasoning(prompt, max_tokens=200 * len(batch))
        metrics.add_llm_call(duration)
        ...
        for item in parsed:
            ...
            if action == "CONFIRM":
                ...
                if not dry_run:
                    ...
                    # Re-store the rejected memory with skip_dedup
                    store_memory(
                        entry["new_text"],
        ...
        for item in parsed:
            ...
            if action == "DELETE":
                ...
                if not dry_run:
                    ...
                    with graph._get_conn() as conn:
                        conn.execute(
                            "UPDATE nodes SET confidence = 0.3, accessed_at = ? WHERE id = ?",
                ...
                if not dry_run:
                    ...
                    with graph._get_conn() as conn:
                        conn.execute(
                            "UPDATE nodes SET pinned = 1, confidence = 0.7 WHERE id = ?",
                            (node_id,)
                        )
...
def review_pending_memories(graph: MemoryGraph, dry_run: bool = True,
    ...
    """
    Review all pending memories by calling Opus directly via the Anthropic API.
    ...
    max_tokens = _cfg.models.max_output('high')
  ...
  {{"action": "MERGE", "merge_ids": ["uuid1", "uuid2"], "merged_text": "consolidated"}}
    ...
    for batch_num, batch_rows in enumerate(batches, 1):
        ...
        try:
            ...
            response_text, duration = call_anthropic(
...
_LOCK_FILE = Path(__file__).parent.parent.parent / "data" / ".janitor.lock"
...
def _run_task_optimized_inner(task: str, dry_run: bool = True, incremental: bool = True,
    ...
    # ⚠️ API KEY CHECK — janitor needs LLM API keys to function.
    # It MUST be run through the bot's heartbeat (which passes keys),
    # NOT from a standalone cron/launchd job.
    _has_api_key = bool(
    ...
    if not _has_api_key and task not in ("embeddings", "cleanup", "rag"):
        print("⚠️" * 10)
        print("⚠️  WARNING: No LLM API key found in environment!")
        print("⚠️  The janitor needs an API key for review, dedup, and decay tasks.")
        print("⚠️  Run via your bot's heartbeat (which passes the key) — NOT standalone cron/launchd.")
        print("⚠️  Or set ANTHROPIC_API_KEY in your environment or .env file.")
        print("⚠️" * 10)
        print()
    ...
    try:
        ...
        if task in ("rag", "all") and _system_enabled_or_skip("rag", "Task 7: RAG Reindex") and not _skip_if_over_budget("Task 7: RAG Reindex", 15):
            ...
            try:
                ...
                docs_dir = str(WORKSPACE / _cfg.rag.docs_dir)
                ...
                if _cfg.projects.enabled:
                    for proj_name, proj_defn in _cfg.projects.definitions.items():
                        proj_dir = str(WORKSPACE / proj_defn.home_dir)
...
if __name__ == "__main__":
    ...
    stats_file = WORKSPACE / "logs" / "janitor-stats.json"
    usage = get_token_usage()
    stats_data = {
