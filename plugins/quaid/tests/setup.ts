...
// Test database path with process ID to avoid conflicts
export const TEST_DB = `/tmp/test-memory-${process.pid}.db`
...
export class TestMemoryInterface {
  ...
  async callPython(command: string, args: string[] = []): Promise<string> {
    return new Promise((resolve, reject) => {
      const proc = spawn("python3", [PYTHON_SCRIPT, command, ...args], {
        env: { ...process.env, MEMORY_DB_PATH: this.dbPath, MOCK_EMBEDDINGS: "1" },
      })
  ...
  async search(query: string, owner: string = 'testuser', limit: number = 5, minSimilarity: number = 0.3): Promise<any[]> {
    ...
    // Parse output format: "[0.84] [fact][V][C:0.5] text |ID:id|T:created|P:privacy|O:owner"
    const lines = result.trim().split('\n').filter(line => line.length > 0)

    return lines.map((line, index) => {
      // Match pattern with optional flags, confidence, and metadata suffix
      const match = line.match(/^\[([0-9.]+)\]\s+\[([^\]]+)\]((?:\[[^\]]*\])*)\s+(.+?)\s*\|ID:([^|]+)(?:\|T:[^|]*)?(?:\|P:[^|]*)?(?:\|O:.*)?$/)
      if (match) {
...
export async function createTestMemory(): Promise<TestMemoryInterface> {
  const memory = new TestMemoryInterface(TEST_DB)
