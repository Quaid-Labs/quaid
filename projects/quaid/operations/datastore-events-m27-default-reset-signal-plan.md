...
Do not implement runtime code for M27 until:
...
This document is a plan only. It does not approve daemon start/wake/restart
behavior, OpenClaw hook migration, live `payload.transcript_path` reset default
queueing, new signal types, new lifecycle event names, request/default routing
...
M27 selects the reset bridge, but reset is deliberately narrower than the M24 to
...
M27 is not broad reset automation. It does not infer reset backups from a live
...
6. M26 added default queueing only for plain `session.compaction` events with a
   concrete `session_id` and an existing `payload.transcript_path`.
7. Plain `session.reset` events without `daemon_signal.enabled=true` remain
...
8. OpenClaw adapter-level reset paths already write compatible daemon `reset`
   ...
   orphan backup waits, and recent-reset duplicate suppression. M27 must not
...
9. The daemon reset processor already owns reset backup discovery, cursor
   ...
   plain-session rebasing after reset backup. M27 must not move that ownership
...
1. Add a private helper in `core.runtime.events`, near the M22/M24/M25/M26
   ...
   `_maybe_queue_default_reset_signal(event, *, session_id)`. Keep the helper
...
2. The default bridge is selected only for `event.name == "session.reset"`. It
   maps to the existing daemon signal type `reset` only. Do not add default
...
6. Preserve the M26 default compaction bridge exactly. Plain
...
10. Use `core.extraction_daemon.write_signal()` through an in-function import
    inside the default helper. Do not write signal files by hand. Do not import
    or call daemon process lifecycle helpers such as start, wake, stop, or
    restart.
11. Preserve idempotency by relying on existing `write_signal()` dedupe rules
    ...
    duplicate. Do not implement OpenClaw's recent-reset marker suppression in
    `core.runtime.events`.
12. Record compact signal metadata only: bridge provenance, lifecycle event id,
...
16. Preserve M21 daemon observation behavior. When the daemon later processes the
    default-written reset signal, it should follow the same observation path as
    adapter-written and explicit-M22 reset signals.
17. Preserve daemon reset backup/cursor semantics. M27 must not change daemon
