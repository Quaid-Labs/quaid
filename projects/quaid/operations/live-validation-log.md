...
`modules/quaid/tests/livetest/livetest-guide/`, and release approval follows
`docs/RELEASE.md`.
...
| M8: Neither OC nor CC `before_agent_start` injects Quaid TOOLS.md (project CLI guide) into agent context | High | Historical finding: agent created plain folders/files instead of using `quaid project create`/`link`/`registry register`. At the time, CC session-init wrote a single `.claude/rules/quaid-projects.md`; current CC builds write split `.claude/rules/quaid-*.md` files with bounded project context. OC still needs equivalent startup-context coverage through its adapter path. |
