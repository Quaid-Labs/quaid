...
**OpenClaw latency bounds:** OpenClaw `before_prompt_build` auto-injection uses a bounded fast recall lane so a slow memory lookup cannot outlive the prompt turn and pile up background Python bridge processes. The adapter-level hook deadline is 35s. The datastore recall budget defaults to 32s (`QUAID_AUTO_INJECT_RECALL_TIMEOUT_MS`) and the Python bridge kills recall subprocesses at the recall budget plus 1.5s grace (`QUAID_PYTHON_BRIDGE_RECALL_TIMEOUT_GRACE_MS`), capped by the global bridge timeout. The lane requests `vector_basic` plus `graph` at depth 2 and excludes project docs, keeping graph-linked personal memory reachable while preserving the same hard timeout. Auto-injection only runs on turns where OpenClaw emits `before_prompt_build`; a queued message that only reaches `message_received` and `before_agent_reply` cannot receive pre-model memory context.
...
- `rotate_project_logs(projects_dir, **kwargs)` — rotates `PROJECT.log` for all projects under `QUAID_VISIBLE_HOME/projects/`. Call after distillation.
...
> **Location:** `QUAID_HOME/instances/<instance>/data/memory.db`. When OC and CC share a `QUAID_HOME`,
...
- `QUAID_HOME` — hidden root directory containing runtime state and all instances (default: `~/.quaid`). Set once per machine.
