...
function _ensureOpenClawExecApprovalsAllowQuaid() {
  ...
  const patterns = Array.from(new Set([
    ...
    path.join(HIDDEN_HOME, "modules", "quaid", "quaid"),
  ...
  try {
    ...
    for (const pattern of patterns) {
      ...
      normalizedAllowlist.push({
        id: `quaid-${Date.now()}-${normalizedAllowlist.length}`,
        pattern,
        lastUsedAt: Date.now(),
      });
      seen.add(pattern);
    ...
    log.warn(`Could not update OpenClaw exec approvals allowlist: ${String(err)}`);
    return false;
  } finally {
    try {
      if (fs.existsSync(tmpPath)) fs.unlinkSync(tmpPath);
...
`;
  ...
  if (_isPlatform("openclaw")) {
    ...
    if (_ensureOpenClawExecApprovalsAllowQuaid()) {
      log.info("Ensured OpenClaw exec approvals allowlist includes quaid");
    }
